...
#if (!defined(IMLIB_ENABLE_DEBAYER_MAX_SPEED)) || (!defined(IMLIB_ENABLE_DEBAYER_YCBCR_MAX_SPEED))
...
#endif // !IMLIB_ENABLE_DEBAYER_MAX_SPEED || !IMLIB_ENABLE_DEBAYER_YCBCR_MAX_SPEED

// Note that the loaded pointers are shifted to up by 1 to account for the offset
// created by debayering the image.
#if defined(IMLIB_ENABLE_DEBAYER_AWB_MAX_SPEED)
static inline v4x_row_ptrs_t vdebayer_rowptrs_init(const image_t *src, int32_t y) {
#else
static v4x_row_ptrs_t vdebayer_rowptrs_init(const image_t *src, int32_t y) {
#endif // IMLIB_ENABLE_DEBAYER_AWB_MAX_SPEED
...
#else
static v2x_row_ptrs_t vdebayer_quarter_rowptrs_init(const image_t *src, int32_t y) {
#endif // IMLIB_ENABLE_DEBAYER_AWB_MAX_SPEED
...
#if defined(IMLIB_ENABLE_DEBAYER_MAX_SPEED)
...
#endif // IMLIB_ENABLE_DEBAYER_MAX_SPEED
...
#if defined(IMLIB_ENABLE_DEBAYER_YCBCR_MAX_SPEED)
...
#endif // IMLIB_ENABLE_DEBAYER_YCBCR_MAX_SPEED
...
static void vdebayer_bggr_to_grayscale_awb(image_t *src, image_t *dst, image_t *buf,
    ...
    for (int32_t y = 0, y_end = src->h - 1; y < src->h; y += VBAYER_Y_STRIDE) {
        ...
        #else
        for (int32_t x = 0; x < src->w; x += VBAYER_X_STRIDE) {
            v4x_rows_t rows = vdebayer_load_rows(src, rowptrs, x, offsets);
            v128_predicate_t pred = vdebayer_store_pred(src->w, x);
