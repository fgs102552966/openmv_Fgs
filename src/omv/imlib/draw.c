...
static void imlib_set_pixel_aa(image_t *img, int x, int y, int err, int c) {
    ...
    switch (img->pixfmt) {
        ...
        case PIXFORMAT_RGB565: {
            ...
            #ifdef ARM_MATH_DSP
            uint32_t alpha = err >> 3; // 256 -> 32
            uint32_t smuad_alpha = ((alpha << 16) | (32 - alpha));
            uint32_t rgb = (old_c << 16) | (c & 0xffff);
            uint32_t rb = ((rgb >> 1) & 0x7c007c00) | (rgb & 0x001f001f);
            uint32_t g = rgb & 0x07e007e0;
            uint32_t rb_out = __SMUAD(smuad_alpha, rb) >> 5;
            uint32_t g_out = __SMUAD(smuad_alpha, g) >> 5;
            uint32_t new_c = ((rb_out << 1) & 0xf800) | (g_out & 0x07e0) | (rb_out & 0x001f);
            #else
